...
const App = {
  ...
  start: async function () {
    ...
    //   const deployedNetwork2 = FundstorageArtifact.networks[networkId];
      this.meta = new web3.eth.Contract(
       auction_abi, 
       '0xE95c5b44998682F0F9F480059FDF4939C2295755'
      );
      this.escrow = new web3.eth.Contract(
        fund_abi,
        '0xAc2572946D4FBE28D62Cf04382CD9a9a7Fc20F89'
      );
      // get accounts
      const accounts = await web3.eth.getAccounts();
      this.account = accounts[0];
      console.log(this.account);
      let ca = await this.escrow.methods.getCA().call();
      console.log(ca);
      ...
      let assetArray = await this.meta.methods.getAsset(this.account).call();
        if (assetArray.length == 0) {
            let d0 = document.getElementById("token-sect").appendChild(document.createElement("p"));
            d0.innerHTML = "No assets found."
        }
        else {
      ...
      for (var k = 0; k < issuanceRef.length; k++){
        ...
        document.getElementById("errormsg").innerHTML = "Please ensure you have metamask installed and connected to Mumbai Testnet.";
        let e0 = document.getElementById("errormsg").appendChild(document.createElement("div"));
        let e1 = e0.appendChild(document.createElement("div"));
        let e2 = e1.appendChild(document.createElement("p"));
        let e3 = e2.appendChild(document.createElement("a"));
        e3.href = "https://metamask.io/download"
        e3.innerHTML = "Metamask Installation"
        let e4 = e0.appendChild(document.createElement("div"));
        let e5 = e4.appendChild(document.createElement("p"));
        let e6 = e5.appendChild(document.createElement("a"));
        e6.href = "https://docs.matic.network/docs/develop/metamask/config-polygon-on-metamask"
        e6.innerHTML = "Mumbai Testnet configuration for Metamask"
        document.getElementById("address").innerHTML = "Can't locate address";
  ...
  createNewIssue: async function () {
    ...
    if (parseInt(state_1) == 0) {
      $(b7).addClass("badge rounded-pill bg-danger");
      b7.setAttribute("id", "badge" + tmpInfo[0]);
      b7.innerHTML = "Closed";
    } else if (parseInt(state_1) == 1) {
      $(mk.b7).addClass("badge rounded-pill bg-success");
      b7.innerHTML = "Open";
      // let b8 = b3.appendChild(document.createElement("button"))
      // b8.onclick = function()
    } else {
      $(b7).addClass("badge rounded-pill bg-info");
  ...
  createAuction: async function () {
    ...
    await createAuction(id, start, end).send({ from: this.account });
      ...
      $('#' + badgeref).removeClass("badge rounded-pill bg-danger");
      $('#' + badgeref).addClass("badge rounded-pill bg-success")
  ...
  createIssueModal: async function (bondid) {
      let modalref = "tokenModal" + bondid;
      let tmpBondinfo = await this.meta.methods.bondInfo(bondid).call();
      let addr = tmpBondinfo[4];
      let amt = tmpBondinfo[3]
      let batchArray = await this.meta.methods.getBatch(bondid).call();
      ...
      $(tm11).on("click", function (){
        App.send_Funds(bondid,addr, amt);
        App.mintOrder(bondid, tokenInfo[3]);
      })
  },
